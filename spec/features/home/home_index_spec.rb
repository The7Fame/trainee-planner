...
RSpec.describe 'Home', type: :feature do
  ...
  describe 'when user is authenticated' do
    ...
    context 'when click on link Me' do
      before { click_link 'Me' }

      it { expect(page).to have_current_path('/') }
    end

    context 'when click on link Events' do
      before { click_link 'Events' }

      it { expect(page).to have_current_path('/events') }
    end

    context 'when click on link Categories' do
      before { click_link 'Categories' }

      it { expect(page).to have_current_path('/categories') }
    end
  ...
  describe 'when user is not authenticated' do
    ...
    it { expect(page).to have_no_link('Me') }
    it { expect(page).to have_no_link('Events') }
    it { expect(page).to have_no_link('Categories') }
